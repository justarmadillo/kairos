package com.taha.kairos.core.media
...
class MediaFileManager @Inject constructor(
    ...
    private val pendingMediaRoot: File by lazy {
        File(context.cacheDir, "kairos_pending_media").apply {
            if (!exists()) mkdirs()
        }
    }
    ...
    /**
     * Reserve a fresh path. Unsaved media uses the app cache; persisted media
     * uses cases/{caseId} under the backed-up media root.
     */
    fun newCaseMediaFile(caseId: Long, type: MediaType, originalExtension: String? = null): File {
        val dir = if (caseId > 0L) {
            File(mediaRoot, "cases/$caseId").apply { if (!exists()) mkdirs() }
        } else {
            pendingMediaRoot
        }
        val safeOriginalExtension = originalExtension
            ?.lowercase()
            ?.filter(Char::isLetterOrDigit)
            ?.take(MAX_EXTENSION_LENGTH)
            ?.takeIf(String::isNotEmpty)
        val ext = safeOriginalExtension ?: when (type) {
            MediaType.IMAGE -> "jpg"
            MediaType.VIDEO -> "mp4"
            MediaType.AUDIO -> "m4a"
            MediaType.FILE -> "bin"
