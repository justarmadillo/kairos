...
fun PatientCaseScreen(
    ...
    val isBusy = state.isLoadingCase || state.isImportingMedia || state.isSaving

    BackHandler(enabled = isBusy) {
        // The save commit is intentionally non-cancellable.
    }
    ...
    val interactionBlocker = if (!isBusy) {
        Modifier
    } else {
    ...
    Scaffold(
        ...
        when {
            isEditingCase && state.caseLoadFailed -> {
                Box(
                    modifier = Modifier
                        .fillMaxSize()
                    ...
                    Button(onClick = { editCaseId?.let(viewModel::loadCase) }) {
                        Text("Retry")
                    }
                }
            }
                ...
                HorizontalPager(
                    ...
                    when (page) {
                        0 -> NewPatientTab(
                            ...
                            onMediaImportStarted = viewModel::beginMediaImport,
                            onMediaImportFinished = viewModel::endMediaImport,
