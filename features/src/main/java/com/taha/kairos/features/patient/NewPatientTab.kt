...
import com.taha.kairos.core.components.AudioRecorderModal
import com.taha.kairos.core.components.DiagnosisAutocomplete
import com.taha.kairos.core.components.MediaAttachmentSection
import com.taha.kairos.core.components.MediaDisplayItem
import com.taha.kairos.core.components.PhoneInputSection
import com.taha.kairos.core.components.RichNotesEditor
...
fun NewPatientTab(
    ...
    onRemovePhone: (com.taha.kairos.core.model.PatientPhone) -> Unit,
    ...
    onMediaImportStarted: () -> Boolean,
    onMediaImportFinished: () -> Unit,
    ...
    var lastAppliedNotesHtml by remember { mutableStateOf<String?>(null) }

    // Load existing notes into the editor without echoing an empty editor back
    // into the ViewModel while an existing case is still being fetched.
    androidx.compose.runtime.LaunchedEffect(state.notesHtml) {
        if (state.notesHtml != lastAppliedNotesHtml) {
            if (richTextState.toHtml() != state.notesHtml) {
                richTextState.setHtml(state.notesHtml)
            }
            lastAppliedNotesHtml = state.notesHtml
        }
    }
    ...
    val galleryLauncher = rememberLauncherForActivityResult(
        ...
        if (uris.isEmpty() || !onMediaImportStarted()) {
            return@rememberLauncherForActivityResult
        }
        coroutineScope.launch {
            try {
                val copiedMedia = withContext(Dispatchers.IO) {
                    uris.mapNotNull { uri ->
                        var destination: java.io.File? = null
                        try {
                            ...
                            val file = mediaFileManager.newCaseMediaFile(
                                caseId = 0,
                                type = mediaType,
                            ...
                            Triple(file, mediaType, displayName)
                        } catch (_: Exception) {
                            destination?.delete()
                            null
                        }
                ...
                copiedMedia.forEach { (file, mediaType, originalName) ->
                    onAttachFile(file, mediaType, originalName)
                }
            } finally {
                onMediaImportFinished()
            }
        }
    }
    ...
    val fileLauncher = rememberLauncherForActivityResult(
        ...
        coroutineScope.launch {
            try {
                ...
                copiedFiles.forEach { (file, displayName) ->
    ...
    Column(
        ...
        androidx.compose.runtime.LaunchedEffect(richTextState.annotatedString) {
            val editorHtml = richTextState.toHtml()
            if (editorHtml != state.notesHtml) {
                onNotesChange(editorHtml)
            }
        }
        Spacer(Modifier.height(12.dp))
        ...
        val existingDisplayItems = state.existingMedia.map { existing ->
            MediaDisplayItem(
                localId = existing.localId,
                filePath = existing.item.filePath,
                mediaType = existing.item.mediaType,
                durationMs = existing.item.durationMs,
                isPrimary = existing.item.isPrimary,
                originalFileName = existing.item.originalFileName,
            )
        }
        val pendingDisplayItems = state.pendingMedia.map { pm ->
            MediaDisplayItem(
                localId = pm.localId,
        ...
        val displayItems = existingDisplayItems + pendingDisplayItems
...
private fun queryDisplayName(
    ...
    uri: Uri,
): String? = context.contentResolver
